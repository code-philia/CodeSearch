...
# # Calculate mean and variance of confidences for each pair
# for pair in student_teacher_pairs:
#     confidences = [teacher['confidence'] for teacher in pair['teachers']]
#     pair['mean_conf'] = sum(confidences) / len(confidences)
#     pair['conf_var'] = sum((c - pair['mean_conf'])**2 for c in confidences) / len(confidences)

# # Sort by mean confidence of all three teachers
# student_teacher_pairs.sort(key=lambda x: x['mean_conf'], reverse=True)

# # Initialize list to store selected indices for auto labeling
# auto_label_indices = []

# # Track mean confidence values we've already sampled
# sampled_means = set()

# # Get confidence range
# max_mean_conf = student_teacher_pairs[0]['mean_conf']
# min_mean_conf = student_teacher_pairs[-1]['mean_conf']

# # Sample pairs at 0.03 confidence intervals
# current_mean_conf = max_mean_conf
# while current_mean_conf >= min_mean_conf:
#     # Find first pair with mean confidence <= current_mean_conf
#     for i, pair in enumerate(student_teacher_pairs):
#         if pair['mean_conf'] <= current_mean_conf and pair['mean_conf'] not in sampled_means:
#             # Add this pair
#             auto_label_indices.append(i)
#             sampled_means.add(pair['mean_conf'])
            
#             # Find pair with similar mean but higher variance
#             target_mean = pair['mean_conf']
#             best_var_diff = float('-inf')
#             best_var_idx = None
            
#             for j, other_pair in enumerate(student_teacher_pairs):
#                 if abs(other_pair['mean_conf'] - target_mean) < 0.01 and \
#                    other_pair['conf_var'] > pair['conf_var'] and \
#                    j not in auto_label_indices:
#                     var_diff = other_pair['conf_var'] - pair['conf_var']
#                     if var_diff > best_var_diff:
#                         best_var_diff = var_diff
#                         best_var_idx = j
            
#             if best_var_idx is not None:
#                 auto_label_indices.append(best_var_idx)
#             break
            
#     current_mean_conf -= 0.05

# print(f"Selected {len(auto_label_indices)} samples for auto labeling at 0.03 confidence intervals")


# # Set confidence thresholds
# MEAN_CONF_THRESHOLD = 0.5  # Minimum required mean confidence
...
# Load labeled data to get student indices
student_indices = []
try:
    with open('/home/yiming/cophi/projects/fork/CodeBERT/GraphCodeBERT/codesearch/auto_labelling/sorted_labelling_sample_api.jsonl', 'r') as f:
        for i, line in enumerate(f):
            if i >= 49:  # Start from the 49th entry
                entry = json.loads(line)
                student_indices.append(entry['idx'])
except FileNotFoundError:
    print("Could not find sorted_labelling_sample_api.jsonl file")
    exit(1)

print(f"Loaded {len(student_indices)} student indices starting from entry 49")

# Find positions of these students in student_teacher_pairs
auto_label_indices = []
for student_idx in student_indices:
    for i, pair in enumerate(student_teacher_pairs):
        if pair['student_idx'] == student_idx:
            auto_label_indices.append(i)

print(f"Found {len(auto_label_indices)} matching pairs in student_teacher_pairs")
...
for auto_label_ind in auto_label_indices:
    ...
    # 打开日志文件
    with open('/home/yiming/cophi/projects/fork/CodeBERT/GraphCodeBERT/codesearch/auto_labelling/auto_labelling_teachers_confidence.log', 'a') as log_file:
