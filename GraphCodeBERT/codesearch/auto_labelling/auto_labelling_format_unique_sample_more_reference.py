...
os.environ["OPENAI_API_KEY"] = "sk-pbQ1LgOcWGeSSulzVY4jV23FvWLHrakNOzqpG34LGQRKMklU"
...
input_path = "/home/yiming/cophi/projects/fork/CodeBERT/GraphCodeBERT/codesearch/auto_labelling/sorted_labelling_sample_api_student_conf_more_reference_2.jsonl"
idx_list = []
match_list = []
...
student_teacher_pairs_file = "/home/yiming/cophi/projects/fork/CodeBERT/GraphCodeBERT/codesearch/auto_labelling/student_teachers_pairs_more_reference_2.jsonl"
...
if os.path.exists(student_teacher_pairs_file):
    ...
    for student_idx in range(total):
        ...
        if student_idx in idx_list or student_idx in labeled_indices:
            continue
        ...
        # If we found at least 3 teachers with the same pattern, proceed
        ...
        # else:
        #     # If not enough teachers with the same pattern, consider teachers with different patterns but high confidence
        #     for teacher_idx in idx_list:
        #         teacher_pattern = sentence_patterns[str(teacher_idx)]
        #         if teacher_pattern == student_pattern:
        #             continue  # Skip teachers with the same pattern
                
        #         teacher_vector = ast_vectors[teacher_idx]
        #         similarity = np.dot(student_vector, teacher_vector) / (np.linalg.norm(student_vector) * np.linalg.norm(teacher_vector))
                
        #         # Define a threshold for high confidence
        ...
        #     # Only add the pair if there are at least 3 teachers with high confidence
        #     if len(similarities) >= 3:
        #         # Sort by similarity and take top 3
        #         similarities.sort(key=lambda x: x['confidence'], reverse=True)
        #         top_3_teachers = similarities[:3]
    ...
    print(f"Completed! Generated and saved {len(student_teacher_pairs)} student-teacher pairs based on token similarity and high confidence")
...
MEAN_CONF_THRESHOLD = 0.75  # Minimum required mean confidence
LOWEST_CONF_THRESHOLD = 0.65  # Minimum required lowest confidence
...
auto_label_indices = [idx for idx in auto_label_indices 
                     if student_teacher_pairs[idx]['student_idx'] not in idx_list and student_teacher_pairs[idx]['student_idx'] not in labeled_indices]
